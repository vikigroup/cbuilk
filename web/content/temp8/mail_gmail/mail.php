<?php 
	...
	include "content/".$template."/mail_gmail/smtp.php"; 
	...
	$noidung_AltBody='
	Chào bạn! <br>
	Email này là do khách hàng của bạn gửi liên hệ thông qua website <a href="http://'.$row_mail['domain'].'"> http://'.$row_mail['domain'].'</a>  <br />';
...
if (isset($_POST['them'])==true)//isset kiem tra submit
	...
	/*	if ($cap == NULL){$coloi=true; $coloi_hien_cap= "<br />Bạn chưa nhập ký tự giống trong hình ";}*/
		...
		if($coloi==FALSE)
		{
			...
			.'<br> <br><hr>
			Chúng tôi mong bạn thực hiện giao dịch thành công, chúc công việc kinh doanh của bạn ngày càng thuận lợi." <br>';
		
			
			$ng_ten=$tennguoigui_name;
			$ng_email=$nguoigui_from;
			
			$ch_email=$row_guimail['cauhinh_mail_ten'];
			$ch_pass=$row_guimail['cauhinh_mail_mk'];
			
			$nn_ten=$row_mail['name'];
			...
			$kq=SendMail("mail.jbs.vn","info@jbs.vn","info123456",$row_mail['email'],$nn_email,$tieude,$noidung,$nn_ten);
			//$kq=1;
    ...
    <table width="100%">
        ...
        <tr>
            ...
            <td width="70%" height="50px">
                <img style="padding:5px 0; z-index:9999;" src="skin/temp<?php echo $url;?>/scripts/capcha/dongian.php" width="80" class="img-cap" />  
            </td>
        </tr>
