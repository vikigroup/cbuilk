...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update tbl_other set idshop='".$idshop."',name='".$name."', subject='".$link."',  last_modified=now() where id='".$oldid."'";
		}else{
			$sql = "insert into tbl_other ( idshop , cate , name, subject, status , date_added, last_modified ) values ('".$idshop."','1','".$name."','".$link."','1',now(),now()  )";
		}
		if (mysql_query($sql,$conn)){
			...
			if($loi=='' && $hinh!="")	{
				$sqlUpdate = "update tbl_other set image='".$hinh."' where id='".$oldid."'";
	...
	if ($errMsg == '')
		echo '<script>window.location="'.$$host_link_full.'/quantri/index.php?act=link&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
...
<div class="main_cont_body">
    ...
    <div class="frame_cont_body">    
        ...
        <div class="frm_tbl">
        <form method="post" name="frmForm" enctype="multipart/form-data" action="index.php?act=link_m">   
