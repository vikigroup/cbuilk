...
<div class="main_cont_body">
    ...
    <h1 class="title_menu_admin">Liên kết</h1><!-- End .title_menu_admin -->
    ...
    <div class="frame_cont_body">    
        ...
        <div class="filter_num">
            ...
            <div class="filter_search">
                ...
                <div class="add_news_all">                                
                    <a href="index.php?act=link_m" title=""><span class="icon_add_news_all"></span>Thêm mới liên kết</a>
        ...
        $where="1=1 and cate=1 and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and (idshop='{$idshop}')";
       ...
       // $pageindex = createPage(countRecord("tbl_other",$where),"./?act=shop_category&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
        ...
        <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.';echo $errMsg;?>
            <table width="100%" border="1">
                ...
                ?>
                    <tr>
                        <td align="center"><input type="checkbox" name="chk[]" id="chk[]" class="tai_c"  value="<?=$row['id']?>" /></td>
                        <td><?=$row['id']?></td>
                        <td>
							Tên liên kết: <?=$row['name']?><br />
                        	Liên kết    : <?=$row['subject']?>
                        
                        </td>
                        <td align="center"><img src="imgs/layout/anhien_<?=$row['status']?>.png" class="anhien" title="Ẩn hiện"value="<?=$row['id']?>" alt=""/></td>
                        <td align="center">
                            <a href="index.php?act=link&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Xóa"  onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="imgs/layout/xoa.png" alt=""/></a>
                            &nbsp;
                            <a href="index.php?act=link_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Sửa"><img src="imgs/layout/sua.png" alt=""/></a>
                        </td>
                    </tr>
                   <?php }?>
