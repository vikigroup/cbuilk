...
{ 
	...
	$_SESSION['kt_tukhoa_documentr']=$tukhoa;
...
if (isset($_POST['reset'])==true) {
	...
	$_SESSION['kt_tukhoa_documentr']=-1;
}
if($_SESSION['kt_tukhoa_documentr']==NULL){$tukhoa=-1;}
if($_SESSION['kt_tukhoa_documentr']!=NULL){$tukhoa=$_SESSION['kt_tukhoa_documentr'];}
?>
<script>
...
<div class="main_cont_body">
    ...
    <div class="frame_cont_body">    
            ...
            <div class="filter_search">
              ...
              <div class="fils1">
				...
				<div class="add_news_all">                                
                    <a href="index.php?act=download_m" title=""><span class="icon_add_news_all"></span>Thêm mới tài liệu</a>
        ...
        switch ($_GET['action']){
            case 'del' :
                $id = $_GET['id'];
                $r = getRecord("tbl_other","id=".$id);
				if($r['idshop']==$idshop){
					$resultParent = mysql_query("select id from tbl_other where parent='".$id."'",$conn);
					if (mysql_num_rows($resultParent) <= 0){
						@$result = mysql_query("delete from tbl_other where id='".$id."'",$conn);
						if ($result){
        ...
        $where="1=1 and cate=2 and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and (idshop='{$idshop}')";
        
        $MAXPAGE=1;
        $totalRows=countRecord("tbl_other",$where);
        ...
        <input type="hidden" name="act" value="download">
        <?
       // $pageindex = createPage(countRecord("tbl_other",$where),"./?act=shop_category&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
        ...
        <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.';echo $errMsg;?>
            <table width="100%" border="1">
                <thead>
                    <tr>
                        <td width="3%" align="center"><input type="checkbox" name="chkall" id="chkall" onClick="chkallClick(this);"/></td>
                        <td width="3%">ID</td>
                ...
                $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_other where   $where $sortby $direction limit ".($startRow).",".$pageSize;
                $result=mysql_query($sql,$conn);
                $i=0;
                while($row=mysql_fetch_array($result)){
            /*    $parent = getRecord('tbl_other_category','id = '.$row['parent']);*/
                $color = $i++%2 ? "#d5d5d5" : "#e5e5e5";
                ?>
                    <tr>
                        <td align="center"><input type="checkbox" name="chk[]" class="tai_c" value="<?=$row['id']?>" /></td>
                        <td><?=$row['id']?></td>
                        <td> 
                            ...
                            <img src="../../<?=$row['image']?>"  height="80" border="0" class="hinh" />
                            </a>
                            <?php }else{?>
                            <img src="../../<?php echo $noimgs; ?>"  height="80" border="0" class="hinh" />
                            <?php }?>
                        </td>
                        <td>
							Tên:<?=$row['name']?><br />
                            Link: <a href="../<?=$row['image_large']?>"> here </a><br />
                        </td>
                        <td align="center"><img src="imgs/layout/anhien_<?=$row['status']?>.png" class="anhien" title="Ẩn hiện"value="<?=$row['id']?>" alt=""/></td>
                        <td align="center">
                            <a href="index.php?act=download&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Xóa"  onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="imgs/layout/xoa.png" alt=""/></a>
                            &nbsp;
                            <a href="index.php?act=download_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Sửa"><img src="imgs/layout/sua.png" alt=""/></a>
                        </td>
                    </tr>
                   <?php }?>
