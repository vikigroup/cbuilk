...
<div class="main_cont_body">
    ...
    <h1 class="title_menu_admin">Danh sách sản phẩm</h1><!-- End .title_menu_admin -->
    ...
    <div class="frame_cont_body">    
        ...
        <div class="filter_num">
            ...
            <div class="filter_search">
              ...
              <div class="fils1">
                    ...
                    <?=comboCategory1('ddCat','tbl_item_category',getArrayCategory("tbl_item_category"),'slt_txt2',$_REQUEST['cat'],1,$idshop)?>
            ...
            <div class="filter_btn">
                ...
                <div class="add_news_all">                                
                    <a href="index.php?act=item_m" title=""><span class="icon_add_news_all"></span>Thêm mới sản phẩm</a>
        ...
        switch ($_GET['action']){
            case 'del' :
				...
				if($r['idshop']==$idshop){
					$resultParent = mysql_query("select id from tbl_item where parent='".$id."'",$conn);
            ...
            <table width="100%" border="1">
                <thead>
                    <tr>
                        ...
                        <td width="15%"><a class="title" href="<?=getLinkSort(5)?>">Danh mục</a></td>
                        <td width="19%" align="center"><a class="title" href="<?=getLinkSort(12)?>">Nổi bật</a></td>
                ...
                $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_item where parent<>0 and $where $sortby $direction limit ".($startRow).",".$pageSize;
                ...
                ?>
                    <tr>
                        ...
                        <td> 
                            ...
                            <a  title="Click vào xem ảnh">
                            <img src="../<?=$row['image']?>" width="40" height="40" border="0" class="hinh" />
                            </a>
                            <?php }else{?>
                            <img src="../<?php echo $noimgs; ?>" width="40" height="40" border="0" class="hinh" />
                            <?php }?>
                            <?php if($row['image_large']==true){ ?>
                            <a title="Click vào xem ảnh">
                            <img src="../<?=$row['image_large']?>" width="40" height="40" border="0" class="hinh" />
                            </a>
                            <?php }?>
                        </td>
                        <td><?=$row['name']?>-<?=$row['type']?>
                        <br />
                        
                        <style>
						  .hinhthem li { width:45px; float:left;text-align:center; margin-top:2px;}
						  .hinhthem li img { max-width:40px; max-height:40px;}
						  .hinht  tbody tr td  { padding:0; border-right:none;}
						  </style>
						  <ul class="hinhthem">
							<?php
							$hinh=get_records("tbl_ad","idshop='{$idshop}' AND name='' AND iditem=".$row['id'],"id DESC","0,20"," ");
							...
							<li>
                            	<table class="hinht">
                                	<tr>
                                    	<td height="55">
                                        <img src="../<?php echo $row_hinh['image'];?>" />
                                        </td>
                                    </tr>
                                    <tr>
                                    	<td height="10">
                                        <a href="item/image_hid.php?idimage=<?php echo $row_hinh['id'];?>"><img src="imgs/layout/xoa.png" height="10" width="10" style="bottom:0px;" /></a> 
                                        </td>
                                    </tr>
                                </table>
                        ...
                        <td align="center">
                            <a href="index.php?act=item&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Xóa"  onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="imgs/layout/xoa.png" alt=""/></a>
                            &nbsp;
                            <a href="index.php?act=item_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Sửa"><img src="imgs/layout/sua.png" alt=""/></a>
                        </td>
                    </tr>
                   <?php }?>
