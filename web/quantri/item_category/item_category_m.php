...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update tbl_item_category set code='".$code."',idshop='".$idshop."',name='".$name."', parent='".$parent."',subject='".$subject."',detail_short='".$detail_short."',detail='".$detail."', sort='".$sort."', title='".$title."', description='".$description."', keyword='".$keyword."', status='".$status."',last_modified=now(), lang='".$lang."' where id='".$oldid."'";
		}else{
			$sql = "insert into tbl_item_category (code, cate , idshop , name, parent, subject, detail_short, detail, sort ,title, description , keyword, status ,  date_added, last_modified, lang) values ('".$code."',0,'".$idshop."','".$name."','".$parent."','".$subject."','".$detail_short."','".$detail."','".$sort."','".$title."','".$description."','".$keyword."','1',now(),now(),'".$lang."')";
		}
		if (mysql_query($sql,$conn)){
			if(empty($_POST['id'])) $oldid = mysql_insert_id();
			$r = getRecord("tbl_item_category","id=".$oldid);
			
			$kiemtra=getRecord("tbl_item_category","name='".$name."'");
			$kiemtra1=getRecord("tbl_item","name='".$name."'");
	...
	if ($errMsg == '')
		echo '<script>window.location="index.php?act=item_category&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
...
<div class="main_cont_body">
    ...
    <div class="frame_cont_body">    
        ...
        <div class="frm_tbl">
        <form method="post" name="frmForm" enctype="multipart/form-data" action="index.php?act=item_category_m">   
