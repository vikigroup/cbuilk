...
if (isset($_POST['btnSave'])){
	...
	$errMsg .= checkUpload($_FILES["txtImage"],".jpg;.gif;.bmp;.jpeg;.png",500*1024,0);
	$errMsg .= checkUpload($_FILES["txtImageLarge"],".jpg;.gif;.bmp;.jpeg;.png",500*1024,0);

	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update tbl_slider set idshop='".$idshop."',name='".$name."', link='".$link."', status=0 ,last_modified=now() where id='".$oldid."'";
		}else{
			$sql = "insert into tbl_slider ( idshop , name, link, status , date_added, last_modified ) values ('".$idshop."','".$name."','".$link."','1',now(),now()  )";
		}
		if (mysql_query($sql,$conn)){
			if(empty($_POST['id'])) $oldid = mysql_insert_id();
			$r = getRecord("tbl_slider","id=".$oldid);
		
			$hinh=luu_hinh('hinh','../','images/gianhang/slider/',$loi);// root chua hinh upload	

			if($loi=='' && $hinh!="")	{
				$sqlUpdate = "update tbl_slider set image='".$hinh."' where id='".$oldid."'";
	...
	if ($errMsg == '')
		echo '<script>window.location="'.$host_link_full.'/quantri/index.php?act=slider&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
...
<div class="main_cont_body">
    ...
    <div class="frame_cont_body">    
        ...
        <div class="frm_tbl">
        <form method="post" name="frmForm" enctype="multipart/form-data" action="index.php?act=slider_m">   
          ...
          <div> <? if($errMsg!=''){echo '<p align=center class="err">'.$errMsg.'<br></p>';}?>  </div>
            <table>
                ...
                <tr>
                  ...
                  <td> <input name="hinh" type="file" class="" id="hinh"/>&nbsp;&nbsp;<br /></td>
              </tr>
                <tr>
                    <th>&nbsp;</th>
                    <td>
