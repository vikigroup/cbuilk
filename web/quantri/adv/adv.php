    ...
    <div class="frame_cont_body">    
        ...
        <div class="filter_num">
            ...
            <div class="filter_btn">
                ...
                <div class="add_news_all">                                
                    <a href="index.php?act=adv_m" title=""><span class="icon_add_news_all"></span>Thêm mới sản phẩm</a>
        ...
        switch ($_GET['action']){
            case 'del' :
				...
				if($r['idshop']==$idshop){
					$resultParent = mysql_query("select id from tbl_ad where parent='".$id."'",$conn);
        ...
        $where="1=1 and cate <> 1000 and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and (idshop='{$idshop}')";
            ...
            <table width="100%" border="1">
                <thead>
                    <tr>
                        ...
                        <td width="7%" align="center"><a class="title" href="<?=getLinkSort(13)?>">Loại</a></td>
                ...
                $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_ad where   $where $sortby $direction limit ".($startRow).",".$pageSize;
                $result=mysql_query($sql,$conn);
                $i=0;
                while($row=mysql_fetch_array($result)){
                $parent = getRecord('tbl_ad_category','id = '.$row['parent']);
                ...
                ?>
                    <tr>
                        ...
                        <td align="center"><?php if($row['cate']==0) echo "Quảng cáo";elseif($row['cate']==1) echo "Quảng cáo chạy bên trái";elseif($row['cate']==2) echo "Quảng cáo chạy bên phải";elseif($row['cate']==3) echo "Quảng cáo cóc phải dưới footer";?></td>
                        <td align="center"><img src="imgs/layout/anhien_<?=$row['status']?>.png" class="anhien" title="Ẩn hiện"value="<?=$row['id']?>" alt=""/></td>
                        <td align="center">
                            <a href="index.php?act=adv&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Xóa"  onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="imgs/layout/xoa.png" alt=""/></a>
                            &nbsp;
                            <a href="index.php?act=adv_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" title="Sửa"><img src="imgs/layout/sua.png" alt=""/></a>
                        </td>
                    </tr>
                   <?php }?>
