...
$(document).ready(function() {	  
	...
	obj = this;
		$.ajax({
		   url:'status.php',
		   data: 'id='+ id +'&table=tbl_quanhuyen_category',
		   cache: false,
               ...
               // $pageindex = createPage(countRecord("tbl_quanhuyen_category",$where),"./?act=quanhuyen_category&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
                ...
                <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.'?>
                    ...
                    <table width="100%"  class="admin_table">
                                ...
                                <tr align="center" >
                                  <td valign="middle" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="9">  
                                        ...
                                        <select name="ddCat" id="ddCat" class="list_tim_loc"> 
                                            ...
                                            <?php   
											$gt=get_records("tbl_quanhuyen_category","parent=2   and status=1 ","id DESC"," "," ");
					    ...
					    $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_quanhuyen_category where parent<>0 and $where $sortby   limit ".($startRow).",".$pageSize;
