...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			...
			$sql = "insert into tbl_item (name, parent, parent1 , detail, type , price , pricekm , sort, status,  date_added, last_modified, style, title, description, keyword  ) values ('".$name."','".$parent."','".$parent1."','".$detail."','".$loaihinh."','".$price."','".$pricekm."','".$sort."','".$status."',now(),now(),'1','".$title."','".$description."','".$keyword."')";
		} 
	...
	if ($errMsg == ''){
		if($r['type']=="0") echo '<script>window.location="admin.php?act=news&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
		else  echo '<script>window.location="admin.php?act=news&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
	}
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=news_m">
                 ...
                 <table  class="table_chinh">
                    ...
                    <tr>
                      ...
                      <td valign="middle"><select name="ddCat" id="ddCat" class="table_list">
						...
						<?php   
                        $gt=get_records("tbl_shop_category","parent=211 and status=0 ","name COLLATE utf8_unicode_ci"," "," "); //and (idshop='{$idshop}' or '{$idshop}'=-1)
