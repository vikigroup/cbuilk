...
if (isset($_POST['tim'])==true)//isset kiem tra submit
{
	...
	$_SESSION['kt_tukhoa_newscate']=$tukhoa;
...
$(document).ready(function() {	  
	...
	obj = this;
		$.ajax({
		   url:'status.php',
		   data: 'id='+ id +'&table=jbs_news_category',
		   cache: false,
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                ...
                switch ($_GET['action']){
                    case 'del' :
                        $id = $_GET['id'];
                        $r = getRecord("jbs_news_category","id=".$id);
                        $resultParent = mysql_query("select id from jbs_news_category where parent='".$id."'",$conn);
                        if (mysql_num_rows($resultParent) <= 0){
                            @$result = mysql_query("delete from jbs_news_category where id='".$id."'",$conn);
                            if ($result){
				...
				if($parent!=-1) {
					$parenstrt="$parent";
					$where="1=1 and type=0 and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1) and  ( parent in ({$parenstrt}) or   id=$parent)";
				}
				else $where="1=1 and type=0 and (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1)";
			 
			   
			   $where.=" AND ( status='{$anhien}' or '{$anhien}'=-1)  AND ( hot='{$noibat}' or '{$noibat}'=-1)";  
				
                $MAXPAGE=1;
				$totalRows=countRecord("jbs_news_category",$where);
				
                if ($_REQUEST['cat']!='') $where="parent=".$_REQUEST['cat']; ?>
                <form method="POST" action="admin.php?act=jbsnews_category" name="frmForm" enctype="multipart/form-data">
                <input type="hidden" name="page" value="<?=$page?>">
                <input type="hidden" name="act" value="jbsnews_category">
                <?
               // $pageindex = createPage(countRecord("jbs_news_category",$where),"./?act=shop_category&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
                ...
                <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.'?>
                    ...
                    <table width="100%"   class="admin_table">
                        	...
                        	<tr align="center" >
                                  <td valign="middle"  colspan="10">
                                    <center>
                                        ...
                                        <strong>DANH MỤC TIN TỨC - DỊCH VỤ</strong>
                                        </div>
                                    </center>
                                   </td>
                            </tr>
                                ...
                                <tr >
                                  <td valign="middle" align="left" style="background-color:#F0F0F0; height:40px; padding-left:20px" colspan="10"> 
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0  &&  $anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=jbsnews_category&tang=1&anhien=-1&noibat=-1">Tất cả</a>
                                        </div>
                                        <div class="link_loc" style="width:80px; text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>" >
                                        	<a href="admin.php?act=jbsnews_category&tang=1&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Tăng dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($tang==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=jbsnews_category&tang=0&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=<?php echo $_SESSION['kt_noibat']?>">Giảm dần</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=jbsnews_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=1&noibat=<?php echo $_SESSION['kt_noibat'] ?>"> Ẩn </a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($anhien==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=jbsnews_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=0&&noibat=<?php echo $_SESSION['kt_noibat'] ?>">Hiện</a>
                                        </div>
                                        <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==1) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=jbsnews_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=1">Nổi bật</a>
                                        </div>
                                         <div class="link_loc" style="width:80px;text-align:center; padding:3px; border:solid 1px #999; float:left; margin-right:5px;<?php if($noibat==0) echo 'background-color:#FF0; color:#000;';else echo 'background-color:#FFF; color:#FFF;"';?>">
                                        	<a href="admin.php?act=jbsnews_category&tang=<?php echo $_SESSION['kt_tang'] ?>&anhien=<?php echo $_SESSION['kt_anhien'] ?>&noibat=0">ko nổi bật</a>
                                        </div>
                                  </td>
                                </tr>
                            <tr class="admin_tieude_table">
                                  <td colspan="2" align="center"><input type="submit" value="Xóa chọn" name="btnDel" onClick="return confirm('Bạn có chắc chắn muốn xóa ?');" class="button"></td>
                                  <td colspan="6" align="center"><span class="PageNum"><?php echo pagesListLimit($totalRows,$pageSize);?></span></td>
                                  <td align="center"> <a href="?act=jbsnews_category_m"><img src="images/them.png" alt="" width="48" height="48" border="0" /></a></td>
                          </tr>
                            <tr class="admin_tieude_table">
                                <td align="center">
                                ...
                                <td align="center"><span class="title"><a class="title" href="<?=getLinkSort(3)?>">Tên </a></span></td>
                                <td align="center"><a class="title" href="<?=getLinkSort(4)?>">Thuộc danh mục</a></td>
                                <td align="center"><a class="title" href="<?=getLinkSort(10)?>">Thứ tự </a></td>
                                <td align="center"><a class="title" href="<?=getLinkSort(15)?>">Tiêu biểu</a></td>
                                <td align="center"><span class="title"><a class="title" href="<?=getLinkSort(11)?>">Ẩn hiện</a></span></td>
						...
						$sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from jbs_news_category where parent<>0 and $where $sortby limit ".($startRow).",".$pageSize;
						$result=mysql_query($sql,$conn);
						$i=0;
						while($row=mysql_fetch_array($result)){
						$parent = getRecord('jbs_news_category','id = '.$row['parent']);
						...
						?>
                            <tr>
                                ...
                                <td align="center">
                                    <a href="admin.php?act=jbsnews_category_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"><img src="images/icon3.png"/></a>
                                    <a  title="Xóa" href="admin.php?act=jbsnews_category&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a>
                                </td>
                            </tr>
                         <?php }?>  
                         <tr>
