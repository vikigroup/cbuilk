...
if (isset($_POST['btnSave'])){
	...
	$aa=0;
	if(isset($_POST['id']))if(get_field("tbl_video","name",$name,"id")!="" && $_POST['id']!=get_field("tbl_video","name",$name,"id")) $aa=1;

	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			  $sql = "update tbl_video set idshop='".$idshop."',detail_short='".$detail_short."',detail='".$detail."',name='".$name."', link='".$link."',  last_modified=now() where id='".$oldid."'";
		}else{
			  $sql = "insert into tbl_video ( idshop , name, detail_short , detail , link, status , date_added, last_modified ) values ('".$idshop."','".$name."','".$detail_short."','".$detail."','".$link."','1',now(),now()  )";
		}
		if (mysql_query($sql,$conn)){
			...
			$result = update("tbl_video",$arrField,"id=".$oldid);
			...
			if($sqlUpdateField!='')	{
				$sqlUpdate = "update tbl_video set $sqlUpdateField where id='".$oldid."'";
	...
	if ($errMsg == '')
		 echo '<script>window.location="admin.php?act=videouser&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>' ;
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
