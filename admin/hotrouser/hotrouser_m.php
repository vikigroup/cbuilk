...
if (isset($_POST['btnSave'])){
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			...
			$sql = "insert into tbl_support (name, nickyahoo , sort, status,  date_added, last_modified) values ('".$name."','".$nickyahoo."','".$sort."','1',now(),now())";
		}
	...
	if ($errMsg == '')
		echo '<script>window.location="admin.php?act=hotrouser&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
          ...
          <div class="widget-container">
              <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="#">
                        <input type="hidden" name="act" value="hotrouser_m">
