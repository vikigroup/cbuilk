    ...
    <div class="span12">
        <div class="box-widget">
            <div class="widget-head overview-chart clearfix">
                ...
                <form method="POST" action="index.php?act=inforaovat" name="frmForm" enctype="multipart/form-data"  style="margin:0;">
                        <input type="hidden" name="page" value="<?=$page?>">
                        <input type="hidden" name="act" value="inforaovat">
                ...
                <div id="reportrange" class="pull-right tai_form">
                 <a href="?act=inforaovat_m"><input type="submit" name="reset" class="nut_table" value=" Thêm mới danh mục" title="Reset" /></a>&nbsp; | &nbsp;
                </div>
            </div>
            <div class="widget-container">
                <div class="widget-block">
                    ...
                    $startRow = ($pageNum-1) * $pageSize;
                     ...
                     $where="1=1 and  cate=1 and  (id='{$tukhoa}' or name LIKE '%$tukhoa%' or '{$tukhoa}'=-1)";
                    ...
                    <form method="POST" action="index.php?act=inforaovat" name="frmForm" enctype="multipart/form-data">
                   ...
                   // $pageindex = createPage(countRecord("tbl_raovat",$where),"./?act=shop_category&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
                    ...
                    <? if ($_REQUEST['code']==1) $errMsg = 'Cập nhật thành công.'?>
                        ...
                        <table width="100%"  class="data-tbl-boxy table table-striped table-well responsive">
                           ...
                           $sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%i') as dateAdd,DATE_FORMAT(last_modified,'%d/%m/%Y %h:%i') as dateModify from tbl_raovat where  $where $sortby $direction limit ".($startRow).",".$pageSize;
                            ...
                            ?>
                                ...
                                <tr>
                                    ...
                                    <td align="center">
                                        ...
                                        <a href="index.php?act=inforaovat_m&cat=<?=$_REQUEST['cat']?>&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>"><img src="images/icon3.png"/></a>
    
                                        <a  title="Xóa" href="index.php?act=inforaovat&action=del&page=<?=$_REQUEST['page']?>&id=<?=$row['id']?>" onclick="return confirm('Bạn có muốn xoá luôn không ?');" ><img src="images/icon4.png" width="20" border="0" /></a>
