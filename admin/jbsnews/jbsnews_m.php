...
if (isset($_POST['btnSave'])){
	...
	if(check_table("tbl_item","name='".$name."'","id")==false) $subject= vietdecode($name)."-".rand(2,10015);
	else $subject       = vietdecode($name);
	...
	$catInfo       = getRecord('jbs_news', 'id='.$parent);
	if(!$multiLanguage){
		$lang      = $catInfo['lang'];
	}else{
	...
	if ($name=="") $errMsg .= "Hãy nhập tên dịch vụ !<br>";
	if($parent=="" || $parent < 2) $errMsg .="Bạn chưa chọn danh mục!";
	$errMsg .= checkUpload($_FILES["txtImage"],".jpg;.jpeg;.gif;.bmp;.png",1500*1024,0);
	$errMsg .= checkUpload($_FILES["txtImageLarge"],".jpg;.jpeg;.gif;.bmp;.png",1500*1024,0);

	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update jbs_news set name='".$name."',parent='".$parent1."',detail_short='".$detail_short."',detail='".$detail."',sort='".$sort."', status='".$status."', title='".$title."', description='".$description."', keyword='".$keyword."',last_modified=now() where id='".$oldid."'";
		}else{
			$sql = "insert into jbs_news (name, parent , detail_short , detail, title , description , keyword , sort, status,  date_added, last_modified  ) values ('".$name."','".$parent1."','".$detail_short."','".$detail."','".$title."','".$description."','".$keyword."','".$sort."','0',now(),now())";
		}
		if (mysql_query($sql,$conn)){
			if(empty($_POST['id'])) $oldid = mysql_insert_id();
			$r = getRecord("jbs_news","id=".$oldid);
		
			$arrField = array(
			"subject"          => "'".vietdecode($name)."'"
			);// ko them id vao cuoi cho dep
			$result = update("jbs_news",$arrField,"id=".$oldid);
			...
			if($sqlUpdateField!='')	{
				$sqlUpdate = "update jbs_news set $sqlUpdateField where id='".$oldid."'";
	...
	if ($errMsg == '')
		echo '<script>window.location="admin.php?act=jbsnews&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
}else{
	if (isset($_GET['id'])){
		$oldid=$_GET['id'];
		$page = $_GET['page'];
		$sql = "select * from jbs_news where id='".$oldid."'";
        ...
        <div class="box-widget">
            ...
            <div class="widget-container">
                <div class="widget-block">
                   ...
                   <form method="post" name="frmForm" enctype="multipart/form-data" action="admin.php?act=jbsnews_m">



            

            <input type="hidden" name="act" value="jbsnews_m">
            ...
            <div> <? if($errMsg!=''){echo '<p align=center class="err">'.$errMsg.'<br></p>';}?>  </div>
                ...
                <table  class="table_chinh">
                    ...
                    <tr>
                      ...
                      <td valign="middle"><select name="ddCat" id="ddCat" class="table_list">
                        ...
                        <?php if($parent!=-1 && $parent!=""){?>
                         <option value="<?php echo $parent ?>"><?php echo get_field('jbs_news_category','id',$parent,'name'); ?></option>
                         <?php }?>
                    ...
                    <tr>
                      ...
                      <td valign="middle"> 
                        <select name="ddCatch" id="ddCatch" class="table_list">
                          ...
                          <option value="<?php echo $parent1 ?>"><?php echo get_field('jbs_news_category','id',$parent1,'name'); ?></option>
                          <?php }?>
                    ...
                    <tr>
                        ...
                        <td width="30%" height="31" valign="middle" class="table_chu">
